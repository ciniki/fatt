...
// -------
//
function ciniki_fatt_offeringLoad($ciniki, $tnid, $offering_id) {
    //
    ...
    $rc = ciniki_core_dbHashQueryTree($ciniki, $strsql, 'ciniki.offerings', array(
        array('container'=>'offerings', 'fname'=>'id', 'name'=>'offering',
            'fields'=>array('id', 'course_id', 'course_name', 'permalink', 'price', 'flags', 'flags_display',
                'start_date_ts', 'start_date', 'date_string', 'location', 'max_seats', 'seats_remaining'),
    ...
    if( !isset($rc['offerings']) || !isset($rc['offerings'][0]) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.fatt.20', 'msg'=>'Unable to find offering'));
    }
    ...
    $strsql = "SELECT ciniki_fatt_offering_instructors.id, "
        ...
        . "WHERE ciniki_fatt_offering_instructors.tnid = '" . ciniki_core_dbQuote($ciniki, $tnid) . "' "
    ...
    if( isset($rc['registrations']) ) {
        ...
        $rc = ciniki_sapos_hooks_invoiceStatus($ciniki, $tnid, array('invoice_ids'=>array_keys($invoice_ids)));
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
    ...
    $rsp['offering']['location_id'] = 0;
    if( isset($rc['dates']) ) {
        $rsp['offering']['dates'] = $rc['dates'];
        if( isset($rc['dates'][0]['date']) ) {
            $rsp['offering']['location_id'] = $rc['dates'][0]['date']['location_id'];
        }
